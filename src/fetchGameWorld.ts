...
const fetchGameWorld = async (chosenGenre: string, chosenCharacter: string, apiKey: string, provider: string, customUrl?: string, customModel?: string) => {
  const maxWords = 80;
...
`;
  const fetchedGameWorld = await chatGPTRequest(gameWorldPrompt, apiKey, provider, customUrl, customModel);
